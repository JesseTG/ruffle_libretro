...
use rust_libretro_sys::{
    retro_hw_context_type, retro_hw_context_type::*,
};
...
impl Ruffle {
    pub(crate) fn get_render_backend(&self) -> Result<(WgpuRenderBackend<TextureTarget>, Box<dyn RenderState>), Box<dyn Error>> {
        let hw_render_callback = self.hw_render_callback.as_ref().ok_or(HwRenderCallbackNotReady)?;
        let environ_cb = self.environ_cb.get();
        let render_state: Box<dyn RenderState> = match hw_render_callback.context_type {
            RETRO_HW_CONTEXT_OPENGL
            | RETRO_HW_CONTEXT_OPENGLES2
            | RETRO_HW_CONTEXT_OPENGLES3
            | RETRO_HW_CONTEXT_OPENGL_CORE
            | RETRO_HW_CONTEXT_OPENGLES_VERSION => Box::new(OpenGlRenderState::new(hw_render_callback.get_proc_address)?),
